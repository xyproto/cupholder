...
func mountDevice(o *textoutput.TextOutput, deviceFilename string) error {
	o.Printf("<darkgray>[<blue>cupholder<darkgray>]\t\t<darkgray>Mounting <yellow>%s<darkgray>... <off>", deviceFilename)
...
func generateMountingHandler(deviceFilenames []string) func(w http.ResponseWriter, req *http.Request) {
	return func(w http.ResponseWriter, req *http.Request) {
		fmt.Fprint(w, "<!doctype html><html><head><title>Cupholder</title><style>html { height: 100%; } body { height: 100%; background: rgb(95,159,221); background-image: linear-gradient(rgba(95,159,221,1) 0%, rgba(95,159,221,1) 6em, rgba(218,192,168,1) 6em, rgba(224,250,255,1) 100%); margin: 3em; overflow: hidden; } h1 { color: orange; text-shadow: 2px 2px 4px #000F; }</style><body>")
		fmt.Fprintf(w, "<h1>%s</h1><br>", versionString)
		for _, deviceFilename := range deviceFilenames {
			fmt.Fprintf(w, "<strong>[cupholder]</strong>&nbsp;&nbsp;<code>Mounting %s...</code> ", deviceFilename)
	...
	if appErr := (&cli.App{
		...
		Action: func(c *cli.Context) error {
			...
			if c.Bool("server") {
				...
				http.HandleFunc("/eject", generateEjectionHandler(deviceFilenames))
				http.HandleFunc("/mount", generateMountingHandler(deviceFilenames))
